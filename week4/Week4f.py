# Mesaj ve dosya seçme diyalogları

import tkinter as tk
from tkinter import messagebox as msg
from tkinter import filedialog as fd

def open_file():
    # Hangi dosya türlerinin seçilebileceğini filtreler
    file_filter = (
        ("All types", "*.*"),
        ("Text file", "*.txt"),
        ("PDF file", "*.pdf")
    )
    # Dosya seçme penceresini açar
    selected_file = fd.askopenfilename(filetypes=file_filter)
    tk.Label(win, text=selected_file).pack()

def print_state():
    # Checkbutton'un işaretli olup olmadığını yazdırır (True/False)
    print(check_state.get())

def exit_app():
    # Çıkış butonuna veya çarpıya basıldığında onay soran kutucuk çıkarır
    dialog_result = msg.askyesno(title="Exit", message="Are you sure you want to exit?")
    if dialog_result:
        win.destroy()

win = tk.Tk()
win.geometry("300x200")
win.title("Week 4")

# Menüdeki onay kutusunun durumunu tutacak değişken
check_state = tk.BooleanVar()

# Pencerenin kırmızı çarpı (kapatma) tuşuna basıldığında exit_app fonksiyonunu çağırır
win.protocol("WM_DELETE_WINDOW", exit_app)

# Ekranın en üstünde yer alacak ana menü çubuğunu oluşturur
menubar = tk.Menu(win)
win.configure(menu=menubar)

# 'File' menüsünü ve içindeki seçenekleri (Yeni, Aç vb.) oluşturur
file_menu = tk.Menu(menubar, tearoff=False)
file_menu.add_command(label="New", accelerator="Ctrl+N", underline=0,
                      command=lambda: msg.showwarning(title="Warning", message="All unsaved progress will be lost."))
file_menu.add_command(label="Open...", underline=0, command=open_file)
file_menu.add_separator()

# 'File' menüsünün altına açılır alt menü (Sub-menu) oluşturur
sub_menu = tk.Menu(file_menu, tearoff=False)
sub_menu.add_command(label="Zoom in")
sub_menu.add_command(label="Zoom out")

# Alt menüyü 'Editor' adıyla File menüsüne ekler
file_menu.add_cascade(label="Editor", menu=sub_menu, underline=3)
file_menu.add_separator()
file_menu.add_command(label="Exit", command=exit_app, underline=1)

# 'Help' menüsünü oluşturur
help_menu = tk.Menu(menubar, tearoff=False)
help_menu.add_checkbutton(label="Send anonymous stats", onvalue=True, offvalue=False,
                          variable=check_state, command=print_state, underline=10)
help_menu.add_separator()
help_menu.add_command(label="About...", underline=0,
                      command=lambda: msg.showinfo(title="About",
                                                   message="SEN4017 - GUI Programming with Python\n\nSpring 2026"))

# Hazırladığımız File ve Help menülerini ana menü çubuğuna yerleştirir
menubar.add_cascade(label="File", menu=file_menu, underline=0)
menubar.add_cascade(label="Help", menu=help_menu, underline=0)

win.mainloop()
